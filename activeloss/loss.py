...
@mlconfig.register
class ReverseCrossEntropyWithProbs(torch.nn.Module):
    def __init__(self, num_classes, scale=1.0):
        super().__init__()
        self.device = device
        self.num_classes = num_classes
...
@mlconfig.register
class ReverseCrossEntropyWithLogits(torch.nn.Module):
    def __init__(self, num_classes, scale=1.0):
        super().__init__()
        self.device = device
        self.num_classes = num_classes
    ...
    def forward(self, pred, labels):
        # pred = torch.log(pred, dim=1)
        pred = torch.clamp(pred, min=1e-7, max=1.0)
        label_one_hot = torch.nn.functional.one_hot(labels, self.num_classes).float().to(pred)
        label_one_hot = torch.clamp(label_one_hot, min=1e-4, max=1.0)
        rce = (-1*torch.sum(pred * torch.log(label_one_hot), dim=1))
        return self.scale * rce.mean()
...
@mlconfig.register
class NormalizedCrossEntropyWithProbs(torch.nn.Module):
    def __init__(self, num_classes, scale=1.0):
        super().__init__()
        self.device = device
        self.num_classes = num_classes
...
@mlconfig.register
class NormalizedCrossEntropyWithLogits(torch.nn.Module):
    def __init__(self, num_classes, scale=1.0):
        super().__init__()
        self.device = device
        self.num_classes = num_classes
...
@mlconfig.register
class NCEandRCEWithProbs(torch.nn.Module):
    def __init__(self, alpha, beta, num_classes):
        super().__init__()
        self.num_classes = num_classes
        self.nce = NormalizedCrossEntropyWithProbs(scale=alpha, num_classes=num_classes)
        self.rce = ReverseCrossEntropyWithProbs(scale=beta, num_classes=num_classes)

    def forward(self, pred, labels):
        return self.nce(pred, labels) + self.rce(pred, labels)
        
@mlconfig.register
class NCEandRCEWithLogits(torch.nn.Module):
    def __init__(self, alpha, beta, num_classes):
        super().__init__()
        self.num_classes = num_classes
        self.nce = NormalizedCrossEntropyWithLogits(scale=alpha, num_classes=num_classes)
        self.rce = ReverseCrossEntropyWithLogits(scale=beta, num_classes=num_classes)
